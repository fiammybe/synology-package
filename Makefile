IMPRESSDIR=src
IMPRESS_INSTALL_DIR=$(INSTALLDIR)/$(IMPRESSDIR)
...
install:
    ...
    [ -d $(IMPRESS_INSTALL_DIR) ] || install -d $(IMPRESS_INSTALL_DIR)
    cp -a $(IMPRESSDIR)/* $(IMPRESS_INSTALL_DIR)

    [ -d $(INSTALLDIR)/ui ] || install -d $(INSTALLDIR)/ui
    cp -a ui/* $(INSTALLDIR)/ui

    # change owner to nobody user/group on DS
    chown -R http:http $(IMPRESS_INSTALL_DIR)
